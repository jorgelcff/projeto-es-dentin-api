...
class DentistaSchemaBase {
  static createBaseSchema() {
    return yup.object().shape({
      ...
      // rg: yup.string().required().matches(/^[0-9]+$/).min(7),
      cro: yup.string().required().matches(/^[0-9]+$/).min(12),
      especialidadeNN: yup.string().oneOf(["Ortodontia", "Periodontia", "Endodontia", "Odontopediatria", "Cirurgia Oral e Maxilofacial", "Implantodontia", "Odontologia Estética"]),
      especialidade2: yup.string().oneOf(["Ortodontia", "Periodontia", "Endodontia", "Odontopediatria", "Cirurgia Oral e Maxilofacial", "Implantodontia", "Odontologia Estética"]).nullable(),
      telefone: yup.string().required().matches(/^[0-9]+$/).min(10),
      dataNasc: yup.date().required(),
      sexo: yup.string().oneOf(['M', 'F', 'N']).required(),
      cidade: yup.string().required(),
      uf: yup.string().required(),
      // bairro: yup.string().required(),
      // rua: yup.string().required(),
